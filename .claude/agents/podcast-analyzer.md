...
- `## Entities` — STRICT: `- <name> :: <type> :: <context> :: [HH:MM:SS]` (exactly 4 fields, ` :: ` as separator, timestamp in brackets). Angle-bracket tokens are placeholders — substitute the actual value, never emit the literal word. Example: `- Chamath Palihapitiya :: Person :: Bestie arguing for Iran JCPOA revival :: [00:34:12]`
- `## Concepts` — STRICT: `- <name> :: <definition> :: [HH:MM:SS]` (exactly 3 fields). Same placeholder convention. Example: `- Sedation hypothesis :: Claim that porn acts as anxiolytic rather than dopamine addiction :: [01:12:45]`
- `## Contradictions` — STRICT: `- <claim> :: <prior_episode_base_filename> :: <resolution> :: [HH:MM:SS]` (exactly 4 fields). Angle-bracket tokens are placeholders — substitute the actual value, never emit the literal word `claim`. `<prior_episode_base_filename>` must already appear in `<vault_path>/index.md` under **Episodes**, or the literal `none` for a contradiction internal to this episode. `<resolution>` ∈ `unresolved | newer-supersedes | both-stand`. Empty section allowed (omit or leave no bullets) when no contradictions were found. Example: `- Sacks now backs JCPOA revival after opposing it on prior episode :: All-In-Iran-Greenland-Energy :: newer-supersedes :: [01:12:04]`
